...
public static class LobbyInfoProvider
{
    ...
    public static LobbyInfo? ParseCode(string code)
    {
        ...
        if (code.Split("-".ToCharArray()).Length != 5) // PCL Nex 大厅
        {
            try
            {
                ...
                return new LobbyInfo
                {
                    ...
                    Type = LobbyType.PCLNex,
                    Ip = "10.114.51.41"
                };
            }
            catch (Exception ex)
            {
                LogWrapper.Error(ex, "Link", "大厅编号解析失败，可能是无效的 PCL Nex 大厅编号: " + code);
            }
        }
        else // 陶瓦
        {
    ...
    public static string? GetUsername() => AllowCustomName
        ? EasyTierPluginSettings.Username.ReplaceNullOrEmpty(NatayarkProfileManager.NaidProfile.Username)
