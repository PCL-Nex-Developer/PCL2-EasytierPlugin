...
using PCL.EasyTierPlugin.Scaffolding.Client.Requests;
...
using static PCL.EasyTierPlugin.Lobby.LobbyInfoProvider;
using static PCL.EasyTierPlugin.Natayark.NatayarkProfileManager;
using LobbyType = PCL.EasyTierPlugin.Scaffolding.Client.Models.LobbyType;
...
public sealed class LobbyController
{
    ...
    public async Task<ScaffoldingClientEntity?> LaunchClientAsync(string username, string code)
    {
        try
        {
            ...
            var localPort = await scfEntity.Tunnel
    ...
    /// <summary>
    /// 退出大厅。这将同时关闭联机隧道和 MC 端口转发，需要自行清理 UI。
    /// </summary>
    public async Task<int> CloseAsync()
    {
        ...
        {
            await ScfClientEntity.Tunnel.DisposeAsync().ConfigureAwait(false);
            await ScfClientEntity.Client.DisposeAsync().ConfigureAwait(false);
            ScfClientEntity = null;
        }
        else if (ScfServerEntity is not null)
        {
            await ScfServerEntity.Tunnel.DisposeAsync().ConfigureAwait(false);
