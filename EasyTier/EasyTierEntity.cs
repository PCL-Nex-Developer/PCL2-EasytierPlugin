...
public class EasyTierEntity
{
    ...
    private async Task<Process> _BuildProcessAsync(bool asHost)
    {
        ...
        args.AddFlag("no-tun")
            ...
            .AddFlagIf(!EasyTierPluginSettings.TryPunchSymmetricNat, "disable-sys-hole-punching")
            .AddFlagIf(!EasyTierPluginSettings.EnableIPv6, "disable-ipv6")
            .AddFlagIf(EasyTierPluginSettings.UseLatencyFirstMode, "latency-first")
            .Add("encryption-algorithm", "aes-gcm")
            .Add("compression", "zstd")
            .Add("default-protocol", EasyTierPluginSettings.ProtocolPreference.ToString().ToLowerInvariant())
            .Add("network-name", _lobby.NetworkName)
            .Add("network-secret", _lobby.NetworkSecret)
            //.Add("relay-network-whitelist", _lobby.NetworkName)
            .Add("machine-id", Identify.LauncherId)
    ...
    private async Task<IReadOnlyList<string>> _GetEtRelayListAsync()
    {
        ...
        var customedNodes = EasyTierPluginSettings.CustomRelayServer.Split(';', StringSplitOptions.RemoveEmptyEntries);
        ...
        var setupRelayList = relays.Select(relay => new { relay, serverType = EasyTierPluginSettings.ServerType })
            .Where(rl =>
    ...
    {
        while (State != EtState.Stopped && EasyTierPluginSettings.EnableDebugOutput)
        {
            using var cliProcess = new Process();
            cliProcess.StartInfo = new ProcessStartInfo
            {
