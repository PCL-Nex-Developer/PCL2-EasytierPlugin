...
public static class LobbyTextHandler
{
    ...
    public static string GetConnectTypeName(ConnectionWay type)
    {
        return Lang.Text(type switch
        {
            ConnectionWay.Local => "Link.Connection.Local",
            ConnectionWay.P2P => "Link.Connection.P2P",
            ConnectionWay.Relay => "Link.Connection.Relay",
            _ => "Link.Connection.Unknown"
        });
    }
