...
public class LobbyService() : GeneralService("lobby", "LobbyService")
{
    ...
    private static readonly SemaphoreSlim _discoveringGate = new(1, 1);
    private static long _discoveringGeneration;
    ...
    public static ConnectionWay CurrentConnectionWay =>
        _LobbyController.ScfClientEntity?.HostInfo.Way ?? ConnectionWay.Unknown;
    ...
    public static async Task DiscoverWorldAsync()
    {
        ...
        long generation;
        await _discoveringGate.WaitAsync().ConfigureAwait(false);
        try
        {
            if (CurrentState is not LobbyState.Initialized && CurrentState is not LobbyState.Idle)
            {
            ...
            if (_discoveringTask is not null)
            {
                try
                {
                    await _discoveringTask.ConfigureAwait(false);
                }
            ...
            token = _discoveringCts.Token;
            generation = Interlocked.Increment(ref _discoveringGeneration);

            _SetState(LobbyState.Discovering);
            await _RunInUiAsync(() => DiscoveredWorlds.Clear()).ConfigureAwait(false);

            discoveryTask = DiscoverWorldCoreAsync(generation, token);
            _discoveringTask = discoveryTask;
        }
        finally
        {
            _discoveringGate.Release();
        }
        ...
        {
            if (generation == Volatile.Read(ref _discoveringGeneration) &&
                !token.IsCancellationRequested &&
                CurrentState == LobbyState.Discovering)
            {
                _SetState(LobbyState.Initialized);
            }
        }
    ...
    private static async Task DiscoverWorldCoreAsync(long generation, CancellationToken token)
    {
        var recordedPorts = new ConcurrentSet<int>();
        var pingTasks = new List<Task>();
        using var listener = new BroadcastListener();

        var handler = new Action<BroadcastRecord, IPEndPoint>((info, _) =>
        {
            var pingTask = Task.Run(async () =>
            {
                ...
                try
                {
                    ...
                    await _RunInUiAsync(() =>
                    {
                        if (generation == Volatile.Read(ref _discoveringGeneration) &&
                            !token.IsCancellationRequested &&
                            DiscoveredWorlds.All(world => world.Port != info.Address.Port))
                        {
                            DiscoveredWorlds.Add(new FoundWorld(worldName, info.Address.Port));
                        }
        ...
        lock (pingTasks)
        {
            pendingPings = pingTasks.ToArray();
        }
        await Task.WhenAll(pendingPings).ConfigureAwait(false);
    }
    ...
    {
        if (CurrentState != LobbyState.Initialized)
        {
            LogWrapper.Warn("LobbyService", $"Rejected lobby creation while state is {CurrentState}.");
            HintWrapper.Show("当前已有联机任务，请先退出当前大厅。", HintTheme.Error);
            return false;
        }
        ...
        try
        {
            ...
            serverEntity.EasyTier.EasyTierProcessExisted += _OnEasyTierUnexpectedExit;
    ...
    public static async Task<bool> JoinLobbyAsync(string lobbyCode, string username)
    {
        if (CurrentState != LobbyState.Initialized)
        {
            LogWrapper.Warn("LobbyService", $"Rejected lobby join while state is {CurrentState}.");
            HintWrapper.Show("当前已有联机任务，请先退出当前大厅。", HintTheme.Error);
            return false;
        }
        ...
        try
        {
            ...
            clientEntity.EasyTier.EasyTierProcessExisted += _OnEasyTierUnexpectedExit;
    ...
    private static void _OnEasyTierUnexpectedExit()
    {
        OnServerException?.Invoke(new InvalidOperationException("EasyTier Core 异常退出。"));
    }
