...
public static class ScaffoldingFactory
{
    ...
    private static readonly string _LobbyVendor = $"PCL.Nex {Basics.VersionName}, EasyTier {EasyTierMetadata.CurrentEasyTierVer}";
    ...
    public static async Task<ScaffoldingClientEntity> CreateClientAsync
        (string playerName, string lobbyCode, LobbyType from, CancellationToken ct = default, string? machineId = null)
    {
        ...
        var etEntity = _CreateEasyTierEntity(info, 0, 0, false, machineId);
        etEntity.Launch();
    ...
    private static EasyTierEntity _CreateEasyTierEntity(
        LobbyInfo lobby,
        int mcPort,
        int port,
        bool asHost,
        string? machineId = null) =>
        new(lobby, mcPort, port, asHost, machineId);
}
