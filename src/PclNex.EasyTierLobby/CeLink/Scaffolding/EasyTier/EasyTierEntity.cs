...
public class EasyTierEntity
{
    ...
    public int ScaffoldingPort => _scfPort;
    ...
    public static void CleanupOrphanedLobbyProcesses()
    {
        ...
        foreach (var process in Process.GetProcessesByName("easytier-core"))
        {
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(commandLine) ||
                    !commandLine.Contains("--network-name=scaffolding-mc-", StringComparison.OrdinalIgnoreCase))
                    continue;

                var parentId = GetParentProcessId(process.Id);
                if (parentId == currentProcess.Id || IsProcessAlive(parentId))
                    continue;

                LogWrapper.Warn("EasyTier", $"Cleaning orphaned lobby EasyTier process: {process.Id}");
                process.Kill(true);
                process.WaitForExit(5000);
            }
            catch (Exception ex)
            {
                LogWrapper.Warn(ex, "EasyTier", $"Failed to inspect or clean EasyTier process {process.Id}");
            }
            finally
            {
                process.Dispose();
    ...
    private static string? GetProcessCommandLine(int processId)
    {
        ...
        return searcher.Get().Cast<System.Management.ManagementObject>()
    ...
    private static int GetParentProcessId(int processId)
    {
        ...
        var value = searcher.Get().Cast<System.Management.ManagementObject>()
            .Select(item => item["ParentProcessId"])
            .FirstOrDefault();
    ...
    public EasyTierEntity(LobbyInfo lobby, int minecraftPort, int scfPort, bool asHost, string? machineId = null)
    {
        _lobby = lobby;
        _machineId = machineId ?? Utils.Secret.Identify.LauncherId;
    ...
    public int Launch()
    {
        ...
        try
        {
            ...
            _etProcess.Exited += (_, _) =>
            {
                State = EtState.Stopped;
                if (Volatile.Read(ref _isStopping) == 0)
                    EasyTierProcessExisted?.Invoke();
            };
        }
        catch (Exception ex)
        {
    ...
    private async Task<Process> _BuildProcessAsync(bool asHost)
    {
        ...
        args.AddFlag("no-tun")
            ...
            .Add("machine-id", _machineId)
            .Add("rpc-portal", _rpcPort.ToString())
            .Add("private-mode", "true")
            .AddFlagIf(Config.Link.RelayType == LinkRelayBehavior.ForceRelay, "disable-p2p");
        ...
        bool requireRemotePeer = false)
    {
        ...
        while (State is EtState.Active && retryCount < 300)
        {
            ...
            {
                State = EtState.Stopped;
                LogWrapper.Error("EasyTierEntity", $"EasyTier exited before the network was ready (code {_etProcess.ExitCode}).");
                return (false, null);
            }
            ...
            var hasRemotePeer = info.Players?.Any(player => !player.IsHost) == true;
            if (info.Host.Ping < 1000 && (!requireRemotePeer || hasRemotePeer))
            {
                State = EtState.Ready;
                LogWrapper.Info("EasyTierEntity", requireRemotePeer
                    ? "EasyTier network is ready and connected to a remote peer."
                    : "EasyTier network is ready.");
