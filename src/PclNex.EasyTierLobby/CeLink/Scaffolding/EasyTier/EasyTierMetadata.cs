using System.IO;
using System.Runtime.InteropServices;
using PCL.Core.App;

namespace PCL.Core.Link.Scaffolding.EasyTier;

public static class EasyTierMetadata
{
    public const string CurrentEasyTierVer = "2.6.4";

    public static string EasyTierFilePath => Environment.GetEnvironmentVariable("PCLNEX_EASYTIER_DIR") is { Length: > 0 } explicitPath
        ? explicitPath
        : Path.Combine(Paths.SharedLocalData, "EasyTier",
            CurrentEasyTierVer,
            $"easytier-windows-{(RuntimeInformation.OSArchitecture == Architecture.Arm64 ? "arm64" : "x86_64")}");
}